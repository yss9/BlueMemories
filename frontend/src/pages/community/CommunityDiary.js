...
const CommunityDiaryPage = () => {
    ...
    const id = routeId || location.state?.id;
    ...
    const handleRefresh = () => {
        if (diary && keywords.length > 0) {
            setCurrentVideoIndex((currentVideoIndex + 1) % keywords.length);
        }
    };
    ...
    const posit = Math.round(positive || 0);
    const neut = Math.round(neutral || 0);
    const negat = Math.max(100 - posit - neut, 0);
    ...
    const currentVideoId = keywords.length > 0
        ? keywords[currentVideoIndex % keywords.length]
        : null;
    ...
    const handleLikeSubmit = async () => {
        try {
            ...
            const nextIsLike = response.data;
            setIsLike(nextIsLike);
            setDiary((prevDiary) => ({
                ...prevDiary,
                likeNum: Math.max((prevDiary.likeNum || 0) + (nextIsLike ? 1 : -1), 0)
            }));
        } catch (error) {
